...
export function StudentCounselingPage() {
  ...
  const { data: counselings = [], isLoading } = useQuery({
    queryKey: ['counselings', 'student', studentId],
    queryFn: () => fetchCounselings(studentId),
    enabled: !!studentId,
  })
  ...
  return (
    <div className="max-w-3xl">
      <div className="mb-6">
        <h1 className="font-headline text-2xl font-bold text-on-surface">상담</h1>
        <p className="text-sm text-on-surface-variant mt-1">
          상담 일정과 이력을 확인할 수 있습니다.
        </p>
      </div>
